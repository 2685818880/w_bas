...
public class Project_Mast {
	...
	Project_Mast() throws Exception {
		...
		rs = dao.findId("w_bas_project", "*", "");
		try {
			while (rs.next()) {// 接收数据库中的内容
				v = new Vector<String>();
				v.addElement(rs.getString("project_id"));
		...
		aTextField = new JTextField("99999", 10);
		panel.add(aTextField);

		panel.add(new JLabel("项目名称:"));
		bTextField = new JTextField("99999", 10);
		...
		addButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if ("".equals(aTextField.getText())
					...
					try {
						if (dao.Exists("w_bas_project", "*", "proNo='"
								+ aTextField.getText() + "'")
								|| dao.Exists("w_bas_project", "*", "proName='"
										+ bTextField.getText() + "'")) {
							JOptionPane.showMessageDialog(null,
									"proNo或projectName已经存在");
						} else {
							int temp = dao.insert("w_bas_project",
									"(proNo,proName)", "('" + aTextField.getText()
											+ "','" + bTextField.getText() + "')");
		...
		updButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int t = 0;
				try {
					t = dao.modify("w_bas_project",
							"proNo='" + aTextField.getText() + "',proName='"
									+ bTextField.getText() + "'",
							"project_id='" + ID.getText() + "'");
				} catch (Exception e1) {
					e1.printStackTrace();
				}
				if (t > 0) {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (exists) {
					int n = JOptionPane.showConfirmDialog(null,
							"该项目已经录入报销,删除后将丢失数据,确认要删除吗?", "确认",
							JOptionPane.YES_NO_OPTION);
					if (n == JOptionPane.YES_OPTION) {
						dao.delete("w_bas_bx",
								"project_id=" + Integer.parseInt(ID.getText())
										+ "");
						t = dao.delete("w_bas_project",
								"project_id=" + Integer.parseInt(ID.getText())
										+ "");
